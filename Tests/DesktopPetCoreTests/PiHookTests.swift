...
/// through the `desktoppet hook` CLI with explicit flags (like the opencode plugin).
final class PiHookTests: XCTestCase {
    ...
    func testPiSpec() {
        ...
        XCTAssertTrue(spec.settingsPath.hasSuffix("/.pi/agent/extensions/desktoppet.ts"))
    ...
    func testPiExtensionContent() {
        let ts = HookInstaller.piExtension(binary: "/x/desktoppet")
    ...
    func testPiExtensionInstallRoundTrip() throws {
        let spec = AgentHooks.spec(for: .pi)!
        let path = tmp("desktoppet.ts")
        let cmd = "\"/x/desktoppet\" hook --agent pi"
        ...
        XCTAssertTrue(source.contains("/x/desktoppet"))
