...
final class ApprovalRoundTripTests: XCTestCase {
    ...
    func testApproveRoundTrip() throws {
        let path = "/tmp/desktoppet-\(UUID().uuidString).sock"
    ...
    func testNoDaemonReturnsAskImmediately() {
        let path = "/tmp/desktoppet-\(UUID().uuidString).sock" // nothing listening here
        let start = Date()
    ...
    func testNonApprovalEventClosesConnectionAfterSend() throws {
        ...
        let dir = NSTemporaryDirectory() + "desktoppet-q-\(UUID().uuidString)"
