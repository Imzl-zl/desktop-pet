...
New optional field on `AgentEvent` (`Sources/DesktopPetCore/AgentEvent.swift`),
...
(`Sources/DesktopPetCore/AgentSession.swift`), default `nil`.
...
- `Tests/DesktopPetCoreTests`: add cases to the existing hook-decode tests
