...
/// DesktopPet's hooks, so install/uninstall refuse instead.
...
/// Installs/removes DesktopPet's hook entries in an agent's config. Claude Code,
...
public enum HookInstaller {
    ...
    static func isOurs(_ command: String) -> Bool {
        command.contains("desktoppet") && command.contains("hook")
    }
    ...
    // {"desktoppet": {Event: [{"hooks": [{"type": "command", "command": ...}]}]}}

    /// The hook-group key DesktopPet owns in an Antigravity hooks.json.
    public static let antigravityGroup = "desktoppet"
    ...
    /// Extracts the desktoppet binary path from a hook command like
    /// `"/path/to/desktoppet" hook --agent opencode` (the first quoted token).
    ...
    static func opencodePlugin(binary: String) -> String {
        ...
        // Reports opencode session lifecycle to DesktopPet's menu bar app.
